...
void Vulkan_DestroyDeviceObjects_GUI();
void Vulkan_DestroyFrame_GUI(VkDevice device, Vulkan_Frame* fd, const VkAllocationCallbacks* allocator);
void Vulkan_DestroyFrameSemaphores_GUI(VkDevice device, Vulkan_FrameSemaphores* fsd, const VkAllocationCallbacks* allocator);
void Vulkan_DestroyFrameRenderBuffers_GUI(VkDevice device, Vulkan_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
void Vulkan_DestroyWindowRenderBuffers_GUI(VkDevice device, Vulkan_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
...
static Vulkan_Data_GUI* Vulkan_GetBackendData_GUI()
{
    return ImGui::GetCurrentContext() ? (Vulkan_Data_GUI*)ImGui::GetIO().BackendRendererUserData : nullptr;
}

static uint32_t Vulkan_MemoryType_GUI(VkMemoryPropertyFlags properties, uint32_t type_bits)
{
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
    ...
    alloc_info.memoryTypeIndex = Vulkan_MemoryType_GUI(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
...
static void Vulkan_SetupRenderState_GUI(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, Vulkan_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
...
void Vulkan_RenderDrawData_GUI(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (wrb->FrameRenderBuffers == nullptr)
    {
        ...
        wrb->FrameRenderBuffers = (Vulkan_FrameRenderBuffers*)IM_ALLOC(sizeof(Vulkan_FrameRenderBuffers) * wrb->Count);
        memset(wrb->FrameRenderBuffers, 0, sizeof(Vulkan_FrameRenderBuffers) * wrb->Count);
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Vulkan_CreateFontsTexture_GUI(VkCommandBuffer command_buffer)
{
    ...
    // Create the Image:
    {
        ...
        alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        alloc_info.allocationSize = req.size;
        alloc_info.memoryTypeIndex = Vulkan_MemoryType_GUI(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, req.memoryTypeBits);
...
static void Vulkan_CreatePipeline_GUI(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, VkRenderPass renderPass, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline, uint32_t subpass)
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    Vulkan_CreateShaderModules_GUI(device, allocator);
...
bool Vulkan_CreateDeviceObjects_GUI()
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    Vulkan_InitInfo_GUI* v = &bd->VulkanInitInfo;
    VkResult err;
    ...
    Vulkan_CreatePipeline_GUI(v->Device, v->Allocator, v->PipelineCache, bd->RenderPass, v->MSAASamples, &bd->Pipeline, bd->Subpass);

    return true;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void    Vulkan_DestroyFontUploadObjects_GUI()
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    Vulkan_InitInfo_GUI* v = &bd->VulkanInitInfo;
    if (bd->UploadBuffer)
...
void    Vulkan_DestroyDeviceObjects_GUI()
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    Vulkan_InitInfo_GUI* v = &bd->VulkanInitInfo;
    Vulkan_DestroyWindowRenderBuffers_GUI(v->Device, &bd->MainWindowRenderBuffers, v->Allocator);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool    Vulkan_Init_GUI(Vulkan_InitInfo_GUI* info, VkRenderPass render_pass)
{
    ...
    Vulkan_CreateDeviceObjects_GUI();

    return true;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void Vulkan_Cleanup_GUI()
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    IM_ASSERT(bd != nullptr && "No renderer backend to shutdown, or already shutdown?");
    ImGuiIO& io = ImGui::GetIO();

    Vulkan_DestroyDeviceObjects_GUI();
    io.BackendRendererName = nullptr;
    io.BackendRendererUserData = nullptr;
    IM_DELETE(bd);
}

void Vulkan_NewFrame_GUI()
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    IM_ASSERT(bd != nullptr && "Did you call Vulkan_Init_GUI()?");
    IM_UNUSED(bd);
}

void Vulkan_SetMinImageCount_GUI(uint32_t min_image_count)
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
...
void Vulkan_RemoveTexture_GUI(VkDescriptorSet descriptor_set)
{
    Vulkan_Data_GUI* bd = Vulkan_GetBackendData_GUI();
    Vulkan_InitInfo_GUI* v = &bd->VulkanInitInfo;
...
VkPresentModeKHR Vulkan_SelectPresentMode_GUI(VkPhysicalDevice physical_device, VkSurfaceKHR surface, const VkPresentModeKHR* request_modes, int request_modes_count)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call Vulkan_LoadFunctions_GUI() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
...
int VulkanH_GetMinImageCountFromPresentMode_GUI(VkPresentModeKHR present_mode)
{
...
void Vulkan_CreateWindowSwapChain_GUI(VkPhysicalDevice physical_device, VkDevice device, Vulkan_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    for (uint32_t i = 0; i < wd->ImageCount; i++)
    {
        ...
        Vulkan_DestroyFrameSemaphores_GUI(device, &wd->FrameSemaphores[i], allocator);
    }
    IM_FREE(wd->Frames);
    ...
    // Create Swapchain
    {
        ...
        wd->Frames = (Vulkan_Frame*)IM_ALLOC(sizeof(Vulkan_Frame) * wd->ImageCount);
        wd->FrameSemaphores = (Vulkan_FrameSemaphores*)IM_ALLOC(sizeof(Vulkan_FrameSemaphores) * wd->ImageCount);
...
void Vulkan_DestroyWindowRenderBuffers_GUI(VkDevice device, Vulkan_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    for (uint32_t n = 0; n < buffers->Count; n++)
        Vulkan_DestroyFrameRenderBuffers_GUI(device, &buffers->FrameRenderBuffers[n], allocator);
